 ...
 * @time 16:19
 */
@OpenApiService(name = "modifyTopicShiled", desc = "设置圈子屏蔽状态", responseType = ResponseTypeEnum.SYN)
public class ModifyTopicShiledService extends AbstractApiService<TopicManagerRequest,ApiResponse> {
	@Resource
	...
	protected void doService (TopicManagerRequest request, ApiResponse response) {
		...
		}else if(request.getIsShield () == null){
			result.setToFail (CommonResultCode.NULL_ARGUMENT_EXCEPTION,"是否屏蔽参数不可为空");
		}else {
			TopicOrder order = new TopicOrder ();
			BeanUtils.copyProperties (request, order);
			result = client.modifyTopicShiled (order);
		}
