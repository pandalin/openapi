...
import com.jvv.openapi.service.customer.user.requset.third.UserThirdLoginRequest;
 ...
 * @time 11:50
 */
@OpenApiService(name = "thirdLogin", desc = "三方登录接口", responseType = ResponseTypeEnum.SYN)
public class UserThirdLoginService extends AbstractApiService<UserThirdLoginRequest,UserThirdResponse> {
	@Autowired
	private UserThirdBindingOrLoginClient client;
	@Autowired
	private TokenUtils tokenUtils;
	@Override
	protected void doService (UserThirdLoginRequest request, UserThirdResponse response) {
		UserThirdBindingOrder order=new UserThirdBindingOrder ();
		BeanUtils.copyProperties (request, order);
		//System.out.println("head:"+order.getHeadImg ());
		UserInfoResult result=client.thirdLogin (order);
