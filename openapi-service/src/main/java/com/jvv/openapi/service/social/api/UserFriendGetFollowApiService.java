 ...
 * @time 11:55
 */
@OpenApiService (name = "getFollowList",desc = "获取用户关注列表" , responseType = ResponseTypeEnum.SYN)
public class UserFriendGetFollowApiService extends AbstractApiService<UserFriendRequest,ApiResponse>{
	...
	protected void doService (UserFriendRequest request, ApiResponse response) {
		...
		if (request.getPageNo () == null || "".equals (request.getPageNo ())) {
			...
			//查找关注类型数据
			order.setFriendType (UserFriendTypeEnum.FOLLOW);
