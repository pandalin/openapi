 ...
 * @time
 */
@OpenApiService(name = "addReminder", desc = "增加一条用户设置信息", responseType = ResponseTypeEnum.SYN)
public class UserReminderAddApiService extends AbstractApiService<UserReminderSettingRequest,ApiResponse> {
	@Resource
	...
	protected void doService (UserReminderSettingRequest request, ApiResponse response) {
		...
		SimpleResult result = userReminderSettingClient.add (order);
