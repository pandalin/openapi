...
/*
 * 修订记录:
 * turalyon@jinvovo.com 2015-12-24 19:56 创建
 *
 */
...
import com.jvv.openapi.core.service.convertor.ApiServiceConversionService;
...
public class ObjectAccessor<T> {
	...
	private final static ConcurrentMap<Class<?>, Map<String, Field>> classMap = Maps
			.newConcurrentMap();
	...
	public static <T> ObjectAccessor<T> of(@NotNull T target) {
	...
	private ObjectAccessor(T target) {
		...
		if (fieldMap == null) {
			...
			ReflectionUtils.doWithFields(this.target.getClass(),
	...
	public String getPropertyValue(String propertyName) throws BeansException {
		...
		try {
			...
			throw new ApiServiceException(ApiServiceResultCode.INTERNAL_ERROR, "property="
																			   + propertyName
																			   + "读取失败");
	...
	public void setPropertyValue(String propertyName, Object newValue) {
		...
		try {
			...
			throw new ParamFormatException(field.getName(), newValue, field.getType()
					.getSimpleName());
		} catch (IllegalAccessException ex) {
			logger.error("属性{}设置失败", field.getType(), ex);
			throw new ParamFormatException(field.getName(), newValue, field.getType()
			...
			throw new ParamFormatException(field.getName(), newValue, field.getType()
																			  .getSimpleName() + (enumOptions != null ?
					enumOptions :
					""));
	...
	private boolean isJSONObject(Field field) {
		...
		return Collection.class.isAssignableFrom(type) || Map.class.isAssignableFrom(type)
			   || type.isArray() || Number.class.isAssignableFrom(type) || type.isPrimitive()
	 ...
	 * @return
	 */
	public Map<String, String> getAllData(@NotNull Predicate<Field> predicate) {
		Assert.notNull(target, "predicate must not be null");
	...
	public Map<String, Object> getAllDataForJsonProcess(@NotNull Predicate<Field> predicate) {
		Assert.notNull(target, "predicate must not be null");
	...
	private void parse(String newValue, Field field) {
		try {
			...
			if (e instanceof JSONException) {
				...
				if (cause instanceof ParamFormatException) {
					ParamFormatException ex = (ParamFormatException) cause;
					String requiredTypeOrFormat = ex.getRequiredTypeOrFormat();
					throw new ParamFormatException(field.getName(), newValue, field.getType()
																					  .getSimpleName() + "."
																			  + requiredTypeOrFormat);
				}
			}
			throw new ParamFormatException(field.getName(), newValue, field.getType()
					.getSimpleName());
	...
	private void checkEnumConvertException(Exception e, String newValue, Field field) {
		...
		if (message != null && message.indexOf("enum") > -1) {
			...
			if (message.contains(noExists)) {
				String msg = "枚举类型:"
							 + message.substring(message.indexOf(noExists) + noExists.length(),
						message.length()) + "不存在";
				throw new ParamFormatException(field.getName(), newValue, msg);
			}
			...
			if (message.contains(parseError)) {
				String enumName = message.substring(
